...
- (void)startWave
{
    ...
    if (_firstWaveLayer == nil) {
        ...
        _firstColorLayer.colors = @[(id)HexRGBAlpha(0xFFCE7D, 0.3).CGColor, (id)HexRGBAlpha(0xFA6A43, 0.3).CGColor];
    ...
    if (_secondWaveLayer == nil) {
        ...
        _secondColorLayer.colors = @[(id)HexRGBAlpha(0xFFCE7D, 0.4).CGColor, (id)HexRGBAlpha(0xFA6A43, 0.4).CGColor];
    ...
    if (_thirdWaveLayer == nil) {
        ...
        _thirdColorLayer.colors = @[(id)HexRGBAlpha(0xFFCE7D, 1).CGColor, (id)HexRGBAlpha(0xFA6A43, 1).CGColor];
...
- (void)animateWave
{
    ...
    waveAmplitude = variable * 13;// 幅度调这个值
}
...
- (void)setCurrentThirdWaveLayerPath
{
    ...
    for (float x = 0.0f; x <= waterWaveWidth ; x++) {
        // 正弦波浪公式
        y = waveAmplitude * sin(waveCycle * x + offsetX + waterWaveWidth / 2) + currentWavePointY;
        CGPathAddLineToPoint(path, nil, x, y);
    }
